...
function MatchHistoryCard({
  ...
  // Match Details
  useEffect(() => {
    console.log('inside playermatches useeffect')
    ...
    Promise.all(
      playerMatches.slice(0, index).map(async (match) => {
      ...
      // matchArray.length === playerMatches.length &&
  ...
  useEffect(() => {
    ...
    if (matchDetails.length === index) {
      ...
      setTimeout(() => {
        console.log('inside skeletonfalse')
        skeletonFalse()
      }, 8000)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
