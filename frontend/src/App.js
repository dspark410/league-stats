...
function App() {
  ...
  const {
    summoner: { notFound, summonerInfo },
  ...
  const getSummInfo = (summonerName, rgn) => {
    if (existRgn.includes(rgn)) {
      ...
      if (notFound) {
        ...
        changeURL(summonerInfo.name, region)
        setNonExist(0)

        const doNotAdd = prevEntries
          .map((entry) => {
            return (
              entry[0].includes(summonerInfo.name) &&
        ...
        summonerInfo.profileIconId = summonerInfo.profileIconId.toString()
  ...
  const changeLeaderBoardDiamondToIron = async (rank, division, page) => {
    ...
    await axios
      .get(`${endpoint}/api/leaderboard/${region}/${rank}/${division}/${page}`)
      .then((res) => {
        res.data
  ...
  const changeLeaderBoardChallengertoMaster = async (tier, region) => {
    ...
    await axios
      .get(`${endpoint}/api/leaderboard/${tier}/${region}`)
      .then((res) => {
        res.data.entries
          .sort((a, b) => b.leaguePoints - a.leaguePoints)
          .forEach((entry, i) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        axios.get(`${endpoint}/api/backupjson`).then((res) => {
  ...
  useEffect(() => {
    ...
    if (location.pathname.includes('summoner')) {
      ...
      const summoner = location.pathname.split('/')[3]
      if (summonerInfo) {
        if (summonerInfo.name.toLowerCase() !== summoner) {
          setLoading(true)
          console.log('running summinfo for location')
          getSummInfo(summoner, region)
        }
      }
    }
